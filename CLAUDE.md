...
- **`design-reference/` ist die alleinige Quelle: portierte Komponenten werden nicht additiv erweitert.** Das Verbot gilt der nach aussen sichtbaren Oberflaeche: keine Prop, die die `.d.ts` nicht kennt, kein zusaetzliches Markup, keine zusaetzliche Klasse — auch nicht als „dokumentierte Abweichung", auch nicht, wenn sie einen echten Bedarf löst. Braucht der Aufrufer mehr, löst er es auf seiner Seite oder der Bedarf geht als Änderungswunsch ins Design-Projekt zurück. Davon zu unterscheiden ist der Austausch einer internen Token-Referenz, der die Oberflaeche unveraendert laesst; ein solcher Fall bleibt eskalationspflichtig und wird nur auf Entscheidung des Inhabers vorgenommen — siehe die naechste Zeile.
...
- **Dokumentierte Abweichung (einzige):** `src/components/Tag/Tag.jsx` legt die Rolle `neutral.soft` auf `--medo-action-neutral` / `--medo-action-neutral-text` statt auf die Brand-Stufen der Referenz. Vom Inhaber entschieden, damit der Chip dem Theme folgt; beide Token tragen hell exakt die Referenzwerte. Nicht zurückbauen. Die Rolle `primary.soft` bleibt bewusst auf den Alias-Stufen, weil die Semantic-Ebene kein Paar mit hell teal-50 kennt — der Bedarf ist ans Design-Projekt gemeldet.
