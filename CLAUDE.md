...
- `<DemoPanel>` ist das erste JSX-Element im Overview-Tab-Content jeder Komponenten-Seite, vor allen `<Section>`-Elementen (Import: `import { DemoPanel } from '../docs/PageLayout'`; API: `component`-Funktion + `controls`-Array, dazu optional ein `presets`-Array benannter Beispiele, siehe `src/docs/DemoPanel.jsx`). Die Controls decken die Varianten/Props aus der `.d.ts` der Komponente ab. Info-Seiten haben kein DemoPanel.
...
- **Portale liegen unter den Komponenten ausschließlich in `Modal`, `Popover` und `Tooltip`, im Portal-Chrome zusätzlich in `src/docs/Overlay.jsx`.** `Menu` entkommt der Beschneidung über `position: fixed` mit `z-index: 1000`, nicht über ein Portal — wer sich auf eine Portal-Einteilung stützt, prüfe sie am Code nach.
...
- **`grep -q $'\r'` erkennt CR in dieser Umgebung nicht** und meldet CRLF-Dateien fälschlich als sauber. Belastbar sind `tr -dc '\r' | wc -c` und `file`. Wer die Zeilenenden mit dem untauglichen Mittel prüft, bekommt genau dann eine Entwarnung, wenn er sie am wenigsten gebrauchen kann.
- **`sed -i` ist unter Git Bash kein sicheres Werkzeug für die CRLF-Dateien des Projekts.** Es entfernt die CR-Zeichen stillschweigend und schreibt die Datei durchgehend auf LF um. Weil `core.autocrlf=true` gesetzt ist, **fällt das im `git diff` nicht auf** — dort stehen weiterhin nur die tatsächlich geänderten Zeilen. Für punktgenaue Änderungen an solchen Dateien ein Node-Skript verwenden, das den Rohtext unangetastet lässt, und danach mit `file` gegenprüfen.
- **Rekursives Löschen ist in dieser Umgebung nicht autonom ausführbar.** Der Klassifikator blockiert es unabhängig vom Werkzeug (`rm -rf`, `Remove-Item -Recurse`) und lässt auch nicht zu, dass sich der Ausführende die Berechtigung selbst einträgt; eine Freigabe im Gespräch hebt die Sperre nicht auf. Wer eine Aufräumarbeit plant, plant den Löschschritt von vornherein als Schritt des Inhabers ein.
...
- `git status` meldet nach `npm run search-index` Änderungen an `searchData.js` und `sectionData.js` ohne Inhaltsdiff — reiner Zeilenenden-Effekt: `scripts/generate-search-index.mjs` schreibt LF, beide Dateien liegen im Arbeitsbaum als CRLF, und `.gitattributes` hat keine Regel für `src/`. `git diff --numstat` gibt Auskunft, bevor daraus ein Befund gemacht wird.
